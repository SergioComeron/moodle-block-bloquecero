<?php
// This file is part of Moodle - https://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <https://www.gnu.org/licenses/>.

/**
 * Spanish language strings for block_bloquecero.
 *
 * @package    block_bloquecero
 * @copyright  2025 Sergio Comerón <info@sergiocomeron.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

$string['abrirseccion'] = 'Abrir sección';
$string['addbibliography'] = 'Añadir entrada de bibliografía';
$string['addbook'] = 'Añadir otro libro';
$string['addsession'] = 'Añadir otra sesión';
$string['bibliography'] = 'Bibliografía';
$string['blocksconfig'] = 'Configuración del bloque';
$string['bloquecero:addinstance'] = 'Añadir un nuevo bloque cero';
$string['bloquecero:managebibliography'] = 'Gestionar bibliografía';
$string['bloquecero:manageguides'] = 'Gestionar guías docentes';
$string['bloquecero:managesessions'] = 'Gestionar sesiones en directo';
$string['bloquecero:myaddinstance'] = 'Añadir un nuevo bloque cero a la página Mi Moodle';
$string['bloquecero:viewcourse'] = 'Permite ver el botón de "mostrar curso"';
$string['bookdescription'] = 'Descripción';
$string['bookname'] = 'Nombre del libro';
$string['bookurl'] = 'URL del libro';
$string['bookurl_help'] = 'Introduzca la URL del libro o recurso (opcional). Si no incluye http:// o https://, se añadirá automáticamente https://';
$string['confirmdeletebibliography'] = '¿Estás seguro de que quieres eliminar esta entrada de bibliografía?';
$string['confirmdeletesession'] = '¿Estás seguro de que quieres eliminar esta sesión?';
$string['duration'] = 'Duración';
$string['duration_help'] = 'Duración de la sesión en horas y minutos';
$string['editbibliography'] = 'Editar bibliografía';
$string['editsession'] = 'Editar sesión';
$string['forum_announcement'] = 'Foro de anuncios';
$string['forum_announcement_help'] = 'Seleccione el foro que se utilizará para anuncios del curso en este bloque.';
$string['forum_estudiantes'] = 'Foro de estudiantes';
$string['forum_estudiantes_help'] = 'Seleccione el foro que se utilizará para discusiones de estudiantes en este bloque.';
$string['forum_tutorias'] = 'Foro de tutorías';
$string['forum_tutorias_help'] = 'Seleccione el foro que se utilizará para tutorías del curso en este bloque.';
$string['addguide'] = 'Añadir guía docente';
$string['confirmdeleteguide'] = '¿Estás seguro de que quieres eliminar esta guía docente?';
$string['editguide'] = 'Editar guía docente';
$string['guide_label'] = 'Etiqueta (opcional)';
$string['guide_label_help'] = 'Texto que se mostrará en el menú para identificar esta guía. Si se deja vacío se usará "Guía docente".';
$string['guides_header'] = 'Guías docentes';
$string['manageguides'] = 'Gestionar guías docentes';
$string['noguidesyet'] = 'No se ha añadido ninguna guía docente todavía.';
$string['guide_url'] = 'URL de la guía';
$string['guide_url_help'] = 'Introduzca la URL de la guía docente. Este enlace estará disponible para los usuarios en el bloque.';
$string['header_bg_img'] = 'Imagen de fondo de cabecera';
$string['header_bg_img_desc'] = 'Descripción de la imagen de fondo de cabecera';
$string['invalidurl'] = 'La URL introducida no es válida';
$string['livesessions'] = 'Sesiones en directo';
$string['directos_livelink'] = 'Sesiones en directo';
$string['directos_recordingslink'] = 'Grabaciones';
$string['managebibliography'] = 'Gestionar bibliografía';
$string['managesessions'] = 'Gestionar sesiones';
$string['maxactivitiespersection'] = 'Máximo de actividades por sección';
$string['maxactivitiespersection_help'] = 'Número de actividades mostradas en una sección; el resto se colapsará';
$string['metachild_notice'] = 'Curso compartido con:';
$string['metacourse_notice'] = 'Los alumnos matriculados en este aula tienen acceso al siguiente curso, donde se encuentra todo el contenido:';
$string['metacourse_notice_title'] = 'AULA METAENLAZADA';
$string['noactivities'] = 'Sin actividades';
$string['nobibliographyyet'] = 'No se ha añadido bibliografía todavía';
$string['nosessionsscheduled'] = 'No hay sesiones programadas';
$string['nosessionsyet'] = 'No se han creado sesiones todavía';
$string['pluginname'] = 'Bloque cero';
$string['saveblockfirst'] = 'Por favor guarde la configuración del bloque primero, luego podrá gestionar las sesiones';
$string['selectedteachers'] = 'Profesores visibles';
$string['selectedteachers_help'] = 'Seleccione qué profesores deben mostrarse en el bloque. Si no se selecciona ninguno, se mostrarán todos.';
$string['sept_announcement_body'] = '<p>Se informa a todos los alumnos que este aula queda habilitada para la <strong>Convocatoria de Septiembre</strong>.</p><p>Desde este momento y hasta el próximo {$a} solo se podrán realizar las actividades de evaluación asociadas a esta convocatoria establecidas por el profesor. Cualquier comunicación oficial se realizará exclusivamente a través del <strong>Foro Convocatoria de Septiembre</strong>.</p>';
$string['sept_announcement_subject'] = 'Aula configurada para la Convocatoria de Septiembre';
$string['sept_forum_name'] = 'Foro Convocatoria de Septiembre';
$string['septembernotice_enable'] = 'Activar modo septiembre';
$string['septembernotice_enable_help'] = 'Si se activa, se mostrará un aviso informativo justo debajo del nombre del curso indicando que el aula está habilitada para la convocatoria de septiembre.';
$string['septembernotice_enddate'] = 'Fecha de cierre de las actividades';
$string['septembernotice_enddate_help'] = 'Fecha hasta la que se ampliarán los plazos (fecha de entrega y fecha límite) de las tareas y el cierre de los cuestionarios del curso al activar el modo septiembre. Se aplica al final del día seleccionado (23:59). Solo se modifican las actividades que ya tuvieran una fecha configurada, y únicamente la primera vez que se activa el modo.';
$string['septembernotice_header'] = 'Modo septiembre';
$string['septembernotice_text'] = 'En esta aula, desde este momento y hasta el próximo {$a} solo se podrán realizar las actividades de evaluación asociadas a la convocatoria de septiembre establecidas por el profesor. El profesor solo enviará comunicados a los alumnos a través del Foro Convocatoria de Septiembre';
$string['septembernotice_title'] = 'AULA HABILITADA PARA LA CONVOCATORIA DE SEPTIEMBRE';
$string['sessionadded'] = 'Sesión añadida correctamente';
$string['sessiondate'] = 'Fecha de la sesión';
$string['sessiondateinthepast'] = 'La fecha de la sesión no puede estar muy en el pasado';
$string['sessiondeleted'] = 'Sesión eliminada correctamente';
$string['sessionname'] = 'Nombre de la sesión';
$string['sessionupdated'] = 'Sesión actualizada correctamente';
$string['synccalendar'] = 'Sincronizar con el calendario de Moodle';
$string['synccalendar_help'] = 'Cuando está habilitado, esta sesión se añadirá al calendario del curso de Moodle';
$string['syncedwithcalendar'] = 'Sincronizado con calendario';
$string['teachercustom'] = 'Configuración del perfil del profesor';
$string['userphone'] = 'Teléfono del profesor';
$string['userphone_help'] = 'Introduzca su número de teléfono para mostrarlo en el perfil del profesor.';
$string['userschedule'] = 'Horario de atención';
$string['userschedule_help'] = 'Introduzca su horario de atención para mostrarlo en el perfil del profesor.';


// Cadenas de accesibilidad.
$string['close'] = 'Cerrar';
$string['completed'] = 'Completada';
$string['completionprogress'] = 'Progreso de finalización';
$string['contactinfo'] = 'Información de contacto de {$a}';
$string['courseforums'] = 'Foros del curso';
$string['coursemenu'] = 'Menú del curso';
$string['courseradar'] = 'Course Radar';
$string['coursesections'] = 'Secciones del curso';
$string['failed'] = 'Suspendida';
$string['more'] = 'más';
$string['moremenu'] = 'Más';

// Tour guiado del bloque.
$string['tour_help'] = 'Ayuda';
$string['tour_next'] = 'Siguiente';
$string['tour_prev'] = 'Anterior';
$string['tour_done'] = 'Hecho';
$string['tour_steplabel'] = 'Paso __C__ de __T__';
$string['tour_menu_title'] = 'Menú del curso';
$string['tour_menu_text'] = 'Accesos rápidos a calificaciones, participantes, radar del curso, bibliografía, guías docentes y más.';
$string['tour_header_title'] = 'Tu curso';
$string['tour_header_text'] = 'El nombre y la imagen de cabecera del curso en el que te encuentras.';
$string['tour_team_title'] = 'Equipo docente y fechas';
$string['tour_team_text'] = 'Consulta el profesorado y las fechas del curso. Pulsa el nombre de un profesor para ver su información de contacto.';
$string['tour_forums_title'] = 'Foros del curso';
$string['tour_forums_text'] = 'Tablón de anuncios y foros de tutorías y de estudiantes. El número indica los mensajes sin leer.';
$string['tour_sections_title'] = 'Secciones del curso';
$string['tour_sections_text'] = 'Navega por las secciones en el carrusel y pulsa una para ver sus actividades.';
$string['tour_sessions_title'] = 'Sesiones en directo';
$string['tour_sessions_text'] = 'Calendario semanal de sesiones en directo. Si el curso tiene clases en directo, verás también enlaces a las sesiones y a las grabaciones.';
$string['tour_activities_title'] = 'Calendario de actividades';
$string['tour_activities_text'] = 'Revisa por semana las actividades con fecha y su estado de entrega o calificación.';
$string['tour_togglecourse_title'] = 'Mostrar u ocultar el curso';
$string['tour_togglecourse_text'] = 'Despliega u oculta el contenido completo del curso bajo el bloque.';
$string['nextsection'] = 'Sección siguiente';
$string['nextsession'] = 'Sesión siguiente';
$string['nextweek'] = 'Semana siguiente';
$string['pending'] = 'Pendiente';
$string['previoussection'] = 'Sección anterior';
$string['previoussession'] = 'Sesión anterior';
$string['previousweek'] = 'Semana anterior';
$string['showless'] = 'mostrar menos';
$string['togglecourse'] = 'Mostrar u ocultar contenido del curso';
$string['unreadposts'] = 'mensajes sin leer';
$string['viewallactivities'] = 'Ver todas las actividades';
$string['viewallsessions'] = 'Ver todas las sesiones';
$string['viewall'] = 'Ver todo';
$string['activities'] = 'Actividades';
$string['teachingteam'] = 'Equipo docente';
$string['showcourse'] = 'mostrar curso';
$string['hidecourse'] = 'ocultar curso';
$string['grades'] = 'Calificaciones';
$string['participants'] = 'Participantes';
$string['teacherguide'] = 'Guía docente';
$string['settings'] = 'Configuración';
$string['noticeboard'] = 'Tablón de anuncios';
$string['current'] = 'Actual';
$string['highlighted'] = 'Destacada';
$string['gantt'] = 'Cronograma';
$string['ganttcoursefilter'] = 'Asignaturas';
$string['ganttfilter'] = 'Mostrar';
$string['ganttfilterassign'] = 'Tareas';
$string['ganttfilterforum'] = 'Foros';
$string['ganttfilterlivesessions'] = 'Sesiones';
$string['ganttfilterquiz'] = 'Cuestionarios';
$string['ganttfilterlesson'] = 'Lecciones';
$string['ganttfilterother'] = 'Otros';
$string['ganttafter'] = 'Tras: {$a}';
$string['ganttfiltersections'] = 'Unidades';
$string['ganttexportpdf'] = 'Exportar PDF';
$string['ganttprintnotice'] = 'Para obtener el resultado correcto, selecciona orientación <strong>Horizontal</strong> en el diálogo de impresión.';
$string['sectionscheduling'] = 'Programación de secciones';
$string['sectionscheduling_desc'] = 'Asigne un periodo de fechas a cada sección. El bloque destacará automáticamente la sección cuyo rango incluya la fecha actual.';
$string['sectionstart'] = 'Fecha de inicio';
$string['sectionend'] = 'Fecha de fin';
$string['sectionscheduling_enable'] = 'Programar esta sección';
$string['sectionend_before_start'] = 'La fecha de fin no puede ser anterior a la fecha de inicio.';
$string['sectiondates_overlap'] = 'Este rango de fechas se solapa con el de otra sección.';
$string['colactivity'] = 'Actividad';
$string['coltype'] = 'Tipo';
$string['coldue'] = 'Vence';
$string['colstatus'] = 'Estado';
$string['duetoday'] = 'Vence hoy';
$string['duetomorrow'] = 'Vence mañana';
$string['dueindays'] = 'En {n} días';
$string['overduedays'] = 'Vencida hace {n} días';
$string['submitted'] = 'Entregada';
$string['graded'] = 'Calificada';
$string['monthnames'] = 'Ene,Feb,Mar,Abr,May,Jun,Jul,Ago,Sep,Oct,Nov,Dic';
$string['daynames'] = 'ene,feb,mar,abr,may,jun,jul,ago,sep,oct,nov,dic';
$string['reusezoomcategories'] = 'Usar categorías de Zoom UDIMA';
$string['reusezoomcategories_desc'] = 'Si está activado, bloquecero se aplica a los mismos cursos que el bloque Zoom UDIMA (su lista de categorías). No hace falta mantener una lista aparte. Si Zoom UDIMA no está instalado o no tiene categorías, no se pinta nada extra.';
